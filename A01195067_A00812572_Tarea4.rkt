...
;si alguna linea esta con puros ceros ya valio queso

;Matriz de donde buscar, que posicion buscar
(define findMyNode
  (lambda (mat posToFind)
    (if (null? mat)
        'not-found
        (if (< posToFind 2)
            (car mat)
            (findMyNode (cdr mat) (- posToFind 1))))))
; te encuentra ls posibles siguientes pasos para avanzar dependiendo del primer elemento
; del renglon
(define findMyPaths
  (lambda (fromRow mat)
    (findMyPaths-T fromRow (cdr (findMe mat (car fromRow))) '() 1 mat)))
    
(define findMyPaths-T
  (lambda (from costs paths cont mat)
    (cond
      ((null? costs) paths)
      ((> (car costs) 0) (findMyPaths-T from (cdr costs) (append (list (append (list (car (findMyNode mat cont))) from)) paths) (+ cont 1) mat))
      (else (findMyPaths-T from (cdr costs) paths (+ cont 1) mat)))))

(define tour
  (lambda (mat)
    (if (null? mat)
        #f
        (hasFullCicle? (getNodes mat) (length mat) mat))))

(define getNodes
  (lambda (mat)
    (if (null? mat)
        '()
        (cons (list (caar mat)) (getNodes (cdr mat))))))
        

;verifica si es un camino compleo
(define hasFullCicle?
  (lambda (lista maxLength mat)
    (cond
      ((null? lista) #f)
      ((and (> (length (car lista)) maxLength) (isCicle? (caar lista) (cdar lista))) #t)
      ((hasRepeated? (car lista)) (hasFullCicle? (cdr lista) maxLength mat))
      ((hasFullCicle? (findMyPaths  (car lista) mat) maxLength mat) #t)
      (else (hasFullCicle? (cdr lista) maxLength mat)))))

; verifica si el elemento recibido esta al final de la lista dada
(define isCicle?
  (lambda (head array)
    (cond
      ((null? array) #f)
      ((and ( = (length array) 1) (equal? head (car array))) #t)
      (else (isCicle? head (cdr array))))))

; verifica si tiene uno o mas elementos repetidos en la lista
(define hasRepeated?
  (lambda (row)
    (if (null? row)
        #f
        (if (isThere? (car row) (cdr row))
            #t
            (hasRepeated? (cdr row))))))

; verifica si se tiene un elemento en la lista
(define isThere?
  (lambda (what where)
    (if (null? where)
        #f
